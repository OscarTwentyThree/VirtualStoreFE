...
export const AdminBillManagmentDetail = () => {
  ...
  const manageBill = async (action) => {
    if (action == 1) {
      SwalAlertWithConfirm(
        "warning",
        "¿Está seguro de confirmar el pago del pedido?",
        "Si",
        "Cancelar"
      ).then(async (result) => {
        if (result.isConfirmed) {
          ...
          SwalAlert("success", "Se el pago la del pedido y esta listo para enviar!!");
          navigate("/admin/bill_management");
        } else {
    ...
    if (action == 4) {
        SwalAlertWithConfirm(
          ...
          if (result.isConfirmed) {
            ...
            SwalAlert("success", "Se acepto la devolución del pedido!!");
            navigate("/admin/bill_management");
          } else {
            SwalAlert("error", "Error al aceptar la devolución");
            return;
          }
        });
      }
      ...
      if (action == 5) {
        SwalAlertWithConfirm(
          "warning",
          "¿Está seguro de rechazar la devolución?",
          "Si",
          "Cancelar"
        ).then(async (result) => {
          if (result.isConfirmed) {
            ...
            SwalAlert("success", "Se rechazo la devolución del pedido!!");
            navigate("/admin/bill_management");
          } else {
            SwalAlert("error", "Error al rechazar la devolución");
            return;
          }
        });
      }
  ...
  const displayActionButtom = () => {
    ...
    if (bill.status.id == 1) {
        return (
          <div className="container">
            <div className="row">
              <div className="col">
                <label style={{ marginLeft: "270px", fontSize: "20px" }}>
                  Pedido enviado esperando confirmación de entrega, no hay acciones disponibles
                </label>
              </div>
              <br />
            </div>
    ...
    if (bill.status.id == 52 && Math.abs(Difference_In_Days) > 30) {
      ...
      if (bill.status.id == 102 || bill.status.id == 152) {
        return (
          <div className="container">
            <div className="row">
              <div className="col">
                <label style={{ marginLeft: "355px", fontSize: "20px" }}>
                  Proceso de devolución finalizado, no hay acciones disponibles
                </label>
              </div>
              <br />
            </div>
        ...
        <div className="card rounded shadow">
          <div className="d-flex flex-row-reverse">
            <button
              ...
              onClick={() => navigate("/admin/bill_management")}
            >
              Volver
            </button>
          </div>
